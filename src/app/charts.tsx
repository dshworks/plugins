...
/**
 * The same universe, counted twenty-two ways.
 *
 * Every site here indexes the same GitHub topic, so the spread is not a
 * disagreement about facts — it is each site's filter policy, printed as if it
 * were one. Ours is in the chart, in pink, with no special pleading: 6,290 is
 * a policy too, and the argument of this page is about the receipt on each
 * row, not about whose number is biggest.
 */
export function ClaimsChart({ sites, topicTotal }: { sites: Site[]; topicTotal: number }) {
  const rows = [...sites].sort((a, b) => b.claims - a.claims);
  const max = Math.max(topicTotal, ...rows.map((r) => r.claims));
  const ROW = 17;
  const LABEL = 168;
  const W = 720;
  ...
  return (
    <Plot>
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={`Plugin counts claimed by ${rows.length} directories of the same GitHub topic, from ${fmt(rows[rows.length - 1].claims)} to ${fmt(rows[0].claims)}. The topic held ${fmt(topicTotal)} repositories on the survey date.`}>
      {/* The population line: what the topic actually holds. Every claim is a
          cut of this, which is the only way to read the bars honestly. */}
      <line
        className="ch-grid"
        x1={LABEL + ((W - LABEL - 4) * topicTotal) / max}
        x2={LABEL + ((W - LABEL - 4) * topicTotal) / max}
        y1="0"
        y2={H - 18}
        strokeDasharray="2 3"
      />
      <text className="ch-axis" x={LABEL + ((W - LABEL - 4) * topicTotal) / max} y={H - 6} textAnchor="end">
        {fmt(topicTotal)} in the topic
      </text>
      {rows.map((r, i) => {
        const y = i * ROW;
        const w = Math.max(1, ((W - LABEL - 4) * r.claims) / max);
        return (
          <g key={r.host} style={{ ["--i" as string]: i }}>
            <text className={`ch-label${r.ours ? "" : " dim"}`} x={LABEL - 8} y={y + 11} textAnchor="end">
              {r.host}
            </text>
            <rect
              className={`ch-bar ch-run${r.ours ? " is-ours" : ""}`}
              x={LABEL}
              y={y + 3}
              width={w}
              height={ROW - 7}
              rx="1"
            >
              <title>{`${r.host} claims ${fmt(r.claims)} (${r.curation})`}</title>
            </rect>
            <text className="ch-axis" x={LABEL + w + 5} y={y + 11}>
              {fmt(r.claims)}
            </text>
          </g>
        );
...
/**
 * Tag distribution as a clickable map.
 *
 * A bar per tag, ordered by size, each one a link into the shelf. It replaces
 * a paragraph that said "the ecosystem is piling up in a few places" with the
 * shape of the pile, which is both shorter and checkable.
 */
